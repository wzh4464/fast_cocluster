/// Parameter sweep for ONM3F on RCV1-all with DiMergeCo
///
/// Usage: cargo run --release --example sweep_rcv1_all_onm3f
...
fn main() {
    ...
    if !results.is_empty() {
        ...
        // Compare with NBVD best (T_p=20, 6x6)
        ...
        for &tp in &[5, 10, 15, 20] {
            if let Some(r) = results.iter().find(|r| r.0 == tp && r.1 == 6 && r.2 == 6) {
                println!("T_p={:2}: NMI={:.4}, ARI={:.4}", tp, r.3, r.4);
            }
        }
    }
