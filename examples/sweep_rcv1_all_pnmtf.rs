/// Parameter sweep for PNMTF on RCV1-all with DiMergeCo
/// Tests T_p, grid size, and regularization parameters (tau, eta, gamma)
///
/// Usage: cargo run --release --example sweep_rcv1_all_pnmtf

use fast_cocluster::atom::pnmtf::PnmtfClusterer;
...
fn run_dimerge(
    ...
    let local = PnmtfClusterer::with_config(make_config(k), tau, eta, gamma);
...
fn main() {
    ...
    // Phase 1: T_p and grid sweep with default regularization (0.1, 0.1, 0.1)
    println!("\n=== Phase 1: T_p and Grid Sweep (tau=eta=gamma=0.1) ===");
    println!("{}", "=".repeat(90));
    ...
    let (tau, eta, gamma) = (0.1, 0.1, 0.1);

    for &(tp, m_blks, n_blks) in &grid_configs {
        print!("{:<6} {:<8} {:<8} ", tp, m_blks, n_blks);
        std::io::Write::flush(&mut std::io::stdout()).unwrap();

        if let Some((nmi, ari, time, clusters)) = run_dimerge(&matrix, &true_labels, k, rows, tp, m_blks, n_blks, delta, tau, eta, gamma) {
            println!("{:>8.4} {:>8.4} {:>10.1} {:>10}", nmi, ari, time, clusters);
            grid_results.push((tp, m_blks, n_blks, nmi, ari, time, clusters));
        } else {
    ...
    // Find best T_p/grid config
    let best_grid = grid_results.iter().max_by(|a, b| a.3.partial_cmp(&b.3).unwrap());
    let (best_tp, best_m, best_n) = if let Some(b) = best_grid {
        println!("\nBest grid config: T_p={}, {}x{}, NMI={:.4}", b.0, b.1, b.2, b.3);
        (b.0, b.1, b.2)
    } else {
        println!("\nNo valid grid results, using defaults");
        (20, 6, 6)
    };

    // Phase 2: Regularization sweep with best T_p/grid
    println!("\n=== Phase 2: Regularization Sweep (T_p={}, {}x{}) ===", best_tp, best_m, best_n);
    println!("{}", "=".repeat(90));
    println!("{:<8} {:<8} {:<8} {:>8} {:>8} {:>10}",
             "tau", "eta", "gamma", "NMI", "ARI", "Time(s)");
    println!("{}", "=".repeat(90));
    ...
    let reg_configs = [
        ...
        (0.5, 0.5, 0.5),
        // Asymmetric
        (0.1, 0.1, 0.01),  // lower gamma
        (0.1, 0.1, 0.5),   // higher gamma
        (0.01, 0.01, 0.1), // lower tau/eta
    ];

    let mut reg_results = Vec::new();

    for &(tau, eta, gamma) in &reg_configs {
        print!("{:<8.2} {:<8.2} {:<8.2} ", tau, eta, gamma);
        std::io::Write::flush(&mut std::io::stdout()).unwrap();

        if let Some((nmi, ari, time, _)) = run_dimerge(&matrix, &true_labels, k, rows, best_tp, best_m, best_n, delta, tau, eta, gamma) {
            println!("{:>8.4} {:>8.4} {:>10.1}", nmi, ari, time);
            reg_results.push((tau, eta, gamma, nmi, ari, time));
        } else {
            println!("{:>8} {:>8} {:>10}", "FAIL", "-", "-");
        }
    }
    ...
    if !reg_results.is_empty() {
        let best = reg_results.iter().max_by(|a, b| a.3.partial_cmp(&b.3).unwrap()).unwrap();
        println!("\n=== Best Overall Configuration ===");
        println!("T_p: {}, Grid: {}x{}", best_tp, best_m, best_n);
        println!("tau: {:.2}, eta: {:.2}, gamma: {:.2}", best.0, best.1, best.2);
        println!("NMI: {:.4}, ARI: {:.4}, Time: {:.1}s", best.3, best.4, best.5);

        println!("\nReference: NBVD best was T_p=20, 6x6 with NMI=0.1518");
    }
