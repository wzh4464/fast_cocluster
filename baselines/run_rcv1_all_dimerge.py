#!/usr/bin/env python3
"""Run DiMergeCo-FNMF on RCV1-all (804K x 47K sparse).

Standalone FNMF already done (NMI=0.297). SCC fails on this sparse data.
This script focuses on DiMergeCo-FNMF with row-only partitioning.
"""
...
def main():
    ...
    seeds = list(range(10))  # 10 seeds for statistical reporting
    results = []

    # --- Standalone FNMF (3 seeds, quick confirmation) ---
    print("\n=== FNMF (standalone, 3 seeds) ===")
    for seed in seeds[:3]:
    ...
    # --- DiMergeCo-FNMF with various configs ---
    configs = [
        (2, 1, 10, "DiMergeCo-FNMF-2x1"),
        (4, 1, 10, "DiMergeCo-FNMF-4x1"),
        (8, 1, 10, "DiMergeCo-FNMF-8x1"),
        (4, 1, 30, "DiMergeCo-FNMF-4x1-t30"),
    ]
    ...
    for m_blocks, n_blocks, t_p, method_name in configs:
        ...
        output = {
            "experiment": "RCV1-all DiMergeCo-FNMF (804K x 47K sparse)",
            "dataset": "rcv1_all",
            "shape": list(X.shape),
            "nnz": X.nnz,
            "k": k,
        ...
        out_path = RESULTS_DIR / "rcv1_all_dimerge_fnmf.json"
