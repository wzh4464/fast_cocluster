...
class NMTF:
	...
	def __init__(self,
				 n_row_clusters=2, n_col_clusters=2, alpha = 0.5+1e-1, 
	...
	def _fit_single(self, X, random_state, y=None) :
		...
		##############  NMTF_alpha  ################loop: MUR------> Multiplactive Update Rules
		change = True
		c_init = float(-np.inf)
		c_list = []
		runtime = []
		...
		while change :
			...
			DG = np.nan_to_num(np.diag(G.sum(axis=0)**-1)) #rank2*rank2
			...
			# Criterion alpha-divergence with convex function    f(z) = 1/alpha(1-alpha)   alpha+ (1-alpha)z - z^{1-alpha}

			z = np.nan_to_num(F@S@G.T/X+self.tol, posinf=0)
			f_z = np.nan_to_num( self.alpha+(1-self.alpha)*z-np.power(z,1-self.alpha) , posinf=0)
			
			c = np.sum(np.multiply(X,f_z))
